...
$$
{-} E_{z}[ \log p_{\theta}(x | z) ]
\approx {-} \log p_{\theta}(x_i | z_i) 
...
- $q_{\phi}(z|x)$는 $\mathcal{N}(\mu_i, \sigma_i^2 I)$로 표현 가능하다
  - Encoder의 출력은 $\mu_i$ 벡터와 $\sigma_i$ 벡터이다.

위 두 가정을 통해 Regularization식을 $\mu_i$ 벡터와 $\sigma_i$ 벡터에 대한 식으로 변형 가능하다. 
$$
KL(q_{\phi}(z_i|x_i) || p_{\theta}(z_i)) 
= KL(\mathcal{N}(\mu_i, \sigma_i^2 I) || p_{\theta}(z_i))
...
- 베이즈 정리를 통해서 $p_{\theta}(x) = \frac{p_{\theta}(x | z) p_{\theta}(z)}{p_{\theta}(z | x)}$ 를 유도할 수 있다.
- $z$는 $x$에서 추출한 latent vector로 생각하고 $p_{\theta}(z) \approx \mathcal{N}(0, I)$로 가정한다.
- $z$를 추출하기 위해 $p_{\theta}(z | x)$를 모방하는 $q_{\phi}(z | x)$인 Encoder를 사용한다.
  - $p_{\theta}(x)$를 최대화하는 문제를 $E_{z \sim q_{\phi}(z|x)}[p_{\theta}(x)]$를 최대화하는 문제로 바꾼다.
